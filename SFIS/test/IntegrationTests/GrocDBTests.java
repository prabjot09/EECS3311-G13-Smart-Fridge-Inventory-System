...
import IntegrationTests.FridgeDBTests.StubFood1;
import IntegrationTests.FridgeDBTests.StubFood2;
	...
	// Password and user values are machine dependant based on your own sql settings
		...
		void GrocAddandLoadTest() {
			...
			try {
				...
				createState.executeUpdate("drop table groceryitem;");

				RealDB db = new RealDB(user, password);
				db.addGrocItem(item1);
				db.addGrocItem(item2);
				List<StoredItem> dlist = new ArrayList<StoredItem>();
				dlist = db.loadGroceryItems();
		...
		void GrocUpdateTest() {
			...
			item1.setExpDate(date);
			item2.setExpDate(date1);
			GroceryList grocery = new GroceryList(flist);
			grocery.setItems(flist);
